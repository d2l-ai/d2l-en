...
In the "internet century", it is not unimaginable to collect millions of data everyday. The application of the [two-sample hypothesis test](https://en.wikipedia.org/wiki/Two-sample_hypothesis_testing) as we introduced in :numref:`sec_statistics` is refered to as *A/B Testing* in the world of internet.
...
1. There is a real discrepancy between the control group and the experiment group.
1. There is no discrepancy between the control group and the experiment group, but it happened to be a rare observation.
