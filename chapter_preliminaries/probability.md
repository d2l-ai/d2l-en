...
something unknown (the _target_)
given something known (the _features_).
...
but to _quantify our uncertainty_.
For example, given some features
describing a patient,
we might want to know _how likely_ they are
...
_Probability_ is the mathematical field
...
In fact, _frequentist_ scholars adhere
to an interpretation of probability
that applies _only_ to such repeatable events.
By contrast _Bayesian_ scholars
...
to start off with different _prior_ beliefs.
_Statistics_ helps us to reason backwards,
...
If the coin is _fair_,
then both outcomes
(heads and tails),
are equally likely.
Moreover if we plan to toss the coin $n$ times
then the fraction of heads
that we _expect_ to see
should exactly match
the _expected_ fraction of tails.
...
Formally, the quantity $1/2$ is called a _probability_
...
to outcomes of interest, called _events_.
...
is called _sampling_.
As you might have guessed,
one natural estimator
is the fraction between
the number of observed _heads_
...
is called the _law of large numbers_
and the _central limit theorem_
...
and call it the _sample space_ or _outcome space_.
Here, each element is a distinct possible _outcome_.
...
_Events_ are subsets of the sample space.
...
A _probability_ function maps events
...
_or_ its complement $\mathcal{A}'$ occurring is 1
...
_and_ its complement $\mathcal{A}'$ occurring simultaneously
...
we were invoking the idea of a _random variable_.
...
takes value $v$, denoted by $X=v$, is an _event_
...
to the _distribution_ of $X$, i.e.,
...
between _discrete_ random variables,
like flips of a coin or tosses of a die,
and _continuous_ ones,
...
In these cases we work with probability _densities_.
...
we must take an _integral_ of the density
over that interval.
...
$A=a$ has to happen _and_ $B=b$ also has to happen.
...
It is called the _conditional probability_,
...
we can derive the famous result called _Bayes' theorem_.
...
encoded in the _prior_ $P(H)$,
and a _likelihood function_ that says how likely
...
how to update the initial _prior_ $P(H)$
in light of the available evidence $E$
to produce _posterior_ beliefs
...
Note that $\sum_a P(A=a|B) = 1$ also allows us to _marginalize_ over random variables. That is, we can drop variables from a joint distribution such as $P(A, B)$. After all, we have that
...
In short, two variables are _independent_
...
that diseases and symptoms are _not_ independent.
...
Two random variables $A$ and $B$ are _conditionally independent_
...
That's because the broken bone _explains away_ why some person is in the hospital
...
we often care about knowing what value to expect _on average_.
This quantity is formally called an _expectation_.
...
In general, the _expectation_ (or average)
...
$f$ might be the _utility_ (happiness)
...
how _risky_ an investment is.
Here, we care not just about the expected value
but how much the actual values tend to _vary_
...
The _variance_ of a random variable is calculated by looking
at the expected value of the _squared_ deviations:
...
useful quantity called the _standard deviation_.
...
and variance for _scalar_ random variables,
...
We resolve the problem by taking expectations of the _outer product_
...
The term _epistemic_ refers to anything concerning _knowledge_
...
based on the efficacy of the test _and_
...
1. Assume that you are an asset manager for an investment bank and you have a choice of stocks $s_i$ to invest in. Your portfolio needs to add up to $1$ with weights $\alpha_i$ for each stock. The stocks have an average return $\boldsymbol{\mu} = E_{\mathbf{s} \sim P}[\mathbf{s}]$ and covariance $\boldsymbol{\Sigma} = \mathrm{Cov}_{\mathbf{s} \sim P}[\mathbf{s}]$.
   ...
   1. Compute the _variance_ of the portfolio.
