...
interactions between queries and keys. As it turns out, distance functions are slightly more expensive to compute than inner products. As such, much of the work has gone into attention functions that are simpler to compute. Let's review the Gaussian attention function for a moment:

$$
\begin{aligned}
\alpha(\mathbf{k}, \mathbf{q}) & = \exp\left(\frac{1}{2} \|\mathbf{k} - \mathbf{q}\|^2 \right) \\
& = \exp\left(\mathbf{k}^\top \mathbf{q} -\frac{1}{2} \|\mathbf{k}\|^2 \right) \cdot \exp\left(-\frac{1}{2} \|\mathbf{q}\|^2 \right)
\end{aligned}
$$

First note that the last term depends on $\mathbf{q}$ only. As such it is identical for all $(\mathbf{q}, \mathbf{k}_i)$ pairs. Normalizing the attention weights to $1$, as is done in :eqref:`eq_normalized_attention`, ensures that this term disappears entirely. Secondly, note that both batch and layer normalization lead to activations that have well-bounded, and often constant norms $\|\mathbf{k}_i\| \approx \mathrm{const.}$. This is the case, for instance, whenever the keys $\mathbf{k}_i$ were generated by a layer norm. As such, we can drop it from the definition of $\alpha$ without any major change in the outcome. 

Lastly, we need to keep the order of magnitude of the arguments in the exponential function under control. Assume that all the elements of the query and the key are independent and identically drawn random variables with zero mean and unit variance. The dot product between both vectors has zero mean and a variance of $d$. To ensure that the variance of the dot product still remains one regardless of vector length, we use the *scaled dot-product attention* scoring function. That is, we rescale the dot-product by $1/\sqrt{d}$. We thus arrive at the first commonly used attention function which is used, e.g., in transformers :cite:`Vaswani.Shazeer.Parmar.ea.2017`:

$$
\begin{align}
\alpha(\mathbf{k}, \mathbf{q}) & = \exp\left(\mathbf{k}^\top \mathbf{q} / \sqrt{d} \right) 
\end{align}
$$
:eqlabel:`eq_dot_product_attention`

Note that $\alpha$ still needs normalizing. Note that we can simplify this further via :eqref:`eq_softmax_attention` to use the softmax function. As it turns out, all popular attention mechanisms use the softmax, hence we will limit ourselves to that in the remainder of this chapter. 

$$\alpha(\mathbf{q}, \mathbf{k}_i) = \mathrm{softmax}(\mathbf{k}_i^\top \mathbf{q} / \sqrt{d}) = \frac{\exp(\mathbf{k}_i^\top \mathbf{q} / \sqrt{d})}{\sum_{j=1} \exp(\mathbf{k}_j^\top \mathbf{q} / \sqrt{d})}.$$
...
$\mathbf{q}^\top \mathbf{k}$ with $\mathbf{q}^\top M \mathbf{k}$ where $M$ is a suitably chosen matrix to translate between both spaces. For now assume that the dimensions match. 
