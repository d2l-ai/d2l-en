...
In this case we initialize $Q_0(s, a)$ to some arbitrary value for all $s \in \mathcal{S}$ and $a \in \mathcal{A}$. Again we have $Q^*(s, a) = \lim_{k \to \infty} Q_k(s, a)$ for all $s \in \mathcal{S}$ and $a \in \mathcal{A}$.
...
def value_iteration(einfo, gamma, num_iters):
    ...
    '''
    env_desc      = einfo['desc'] # 2D array specifying what each grid item means
