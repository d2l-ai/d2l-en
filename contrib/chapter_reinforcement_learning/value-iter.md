...
$$
for all states $s \in \mathcal{S}$. This identity is called the "principle of dynamic programming". It was formulated by Richard Bellman in 1940s and we can remember it as "the remainder of an optimal trajectory is also optimal".
...
* Implementing the Value Iteration algorithm requires that we know the Markov decision process (MDP), e.g., the transition and reward functions, competely.
