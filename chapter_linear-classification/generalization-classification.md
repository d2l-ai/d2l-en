...
by training (linear) neural networks with multiple outputs and softmax activations.
...
assigns to the assigns to the actual labels.
...
1. How many test examples do we need to precisely estimate
   the accuracy of our classifiers on theunderlying population?
...
While :ref:`sec_generalization_basics` introduced 
...
that deep neural networks tend generalize 
remarkably well with far fewer examples (thousands). 
Thus deep learning practitioners often forego
...
When we get to chapter :numref:`chap_perceptrons`,
...
a fresh dataset of examples $D = {(\mathbf{x}_i,y_i)}_{i=1}^m$
that were not used to train the classifier $f$.
The *empirical error* of our classifier $f$ on $D$
is simply the fraction of instances
for which the prediction $f(\mathbf{x}_i)$
disagrees with the true label $y_i$,
and is given by the following expression:

$$\epsilon_D(f) = \frac{1}{m}\sum_{i=1}^m \mathbf{1}(f(\mathbf{x}_i) \neq y_i)$$

By contrast, the *population error*
is the *expected* fraction 
of examples in the underlying population
(some distribution $P(X,Y)$  charachterized
by probability density function $p(\mathbf{x},y)$
for which our classifier disagrees
with the true label:

$$\epsilon =  E_{(\mathbf{x}, \mathbf{y}) \sim P} \mathbf{1}(f(\mathbf{x}) \neq y) = 
\int_{((\mathbf{x}, \mathbf{y}))} \mathbf{1}(f(\mathbf{x}) \neq y) p(\mathbf{x}, \mathbf{y})$$

While $\epsilon$ is the quantity that we actually care about,
we cannot observe it directly any more than we can directly 
observe the average height in a population. 
However, because our test set $D$ 
is statistically representative 
of the underlying population,
we can view $\epsilon_D(f)$ as a statistical
...
and the corresponding estimator $\epsilon_D(f)$ 
...
You may recall that a classical result from probility theory
...
the sample average $\hat{mu}$ is approximately
...
In general, such $O(1/\sqrt(n)$ rates 
are often the best we can hope for in statistics.

Now that we know something about asymptotic rate
at which our test errro $\epsilon_D(f)$ converges to the true error $\epsilon(f)$,
...
when the true error rate is close to $.5$
and can be far lower when it is 
close to $0$ or close to $1$. 
This tells us that the asyptotic standard deviation
of our estimate $\epsilon_D(f)$ of the error $\epsilon(f)$
(over the choice of the $m$ test samples)
cannot be any greater than $\sqrt{.25/n}$.

If we ignore the fact that this rate characterizes
behavior as the test set size appraoches infinity
rather than when we possess finite samples,
this tells us that if we want our test error $\epsilon_D(f)$ 
to approximate the population error $\epsilon(f)$
such that one standard deviation corresponds 
to an interval of $\pm .01$,
then we should collect roughly 2500 samples.
If we want to fit two standard deviations 
in that rane and thus be 95% 
that $\epsilon_D(f) \in \epsilon(f) \pm .01$,
...
making a big deal out of error rate improvements of $.01$ or less. 
Of course, when the error rates are much closer to $0$,
then an improvement of $.01$ can indeed be a big deal.


One pesky feature of our analysis thus far 
is that it really only tells us about asymptotics,
i.e., how the relationship between $\epsilon_D$ and $\epsilon$
...
$$P(\epsilon_D(f) - \epsilon(f) \geq t) < \exp\left( - 2m t^2 \right)$$

Solving for the smallest dataset size 
that would allow us to conclude 
with 95% confidence that the distance $t$
between our estimate $\epsilon_D(f)$
...
you know to evaluate its test error $\epsilon_D(f)$,
...
an approperiate number of examples 
...
Even though the original test set $D$
is still sitting on your server,
you now face two formidable problems.
First, when you collected your test,
...
rested on the assumption that the classifer
...
as a topic of intense interest to learning theorists and statisticans
...
when reporting confidenceintervals,
and to dial up your vigilance more aggressively 
when the stakes are high and your dataset size is low. 
...
and yet this fact seems strangely unsatsifying.
...
we soon find ourselves frustrate, wishing we could
...
between the *empirical error* $\epsilon_S(f_S)$
of a learned classifier $f_S$,
both trained and evaluated 
on the training set $S$,
and the true error $\epsilon(f_S)$
...
Suppose that our learned classifier $f_S$ must be chosen 
among some pre-specified set of functions $\cal{F}$.
...
because their paramters are continuously valued,
we are typically choosing among 
an infinite class of functions $|\cal{F}| = \infty$.
...
the empirical error rates for every classifier in the class $f\in\cal{F}$
will *simultaneously* converge to their true error rates.
...
(among all classifiers in thre class $\cal{F}$)
will be misestimated by more 
than some some small amount $\alpha$.
Clearly, we cannot make such statements
for all model classes $\cal{F}$.
...
On the other hand, a fixed classsifier is useless---it 
generalizes perfectly, but fits neither 
the traning data not the test data. 
...
the difference of between the empirical error 
and the population error as a function 
of the VC dimension and the number of samples. 

$$\Pr\left(R[p, f] - R_\mathrm{emp}[\mathbf{X}, \mathbf{Y}, f] < \alpha\right) \geq 1-\delta
...
The most staightforward way to evaluate a model 
...
and converge at desired $O/\sqrt{n}$ rates as as the test set grows.
...
statistical leanring theorists have developed methods
...
best, minizing the training error,
knowing that it too will perform similalry well
on the holdout data. 
Crucially, any such result must depend 
...
have become braodly useful tools in statistical theory,
...
1. If we wish to estimate the error of a fixed model $f$
   to within $.0001$ with probability greater than 99.9%,
   how many samples do we need?
1. Suppose that somebody else possesses a labeled test set 
   and only makes available the unlabeled inputs $\{\mathbf{x}_i\}$. 
   ...
   and receiving the corresponding error $\epsilon_D(f)$.
   How many models would you need to evaluate 
   before you leaked the entire test set 
   and thus could appear to have error $0$,
   regardless of your true error?
1. What is the VC dimension of the class of 5th order polynomials? 
1. What is the VC dimension of axis-aligned rectangles on 2D data?
