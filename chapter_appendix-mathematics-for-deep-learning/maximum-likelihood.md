...
## Exercises
1. Suppose that you know that a random variable has density $\alpha e^{-\alpha x}$ for some value $\alpha$.  You obtain a single observation from the random variable which is the number $3$.  What is the maximum likelihood estimate for $\alpha$?
