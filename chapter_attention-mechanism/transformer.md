...
Let us implement the multi-head attention in MXNet. Assume that the multi-head attention contain the number heads `num_heads` $=h$, the hidden size `hidden_size_qkv` $=p_q=p_k=p_v$ are the same for the query, the key, and the value dense layers, and the output feature size $d_o$ is referred to as `hidden_size_out`. Note in practice, we often use $\frac{d_o}{h} = p_q=p_k=p_v$, that is the final dense weights $\mathbf W_o\in\mathbb R^{d_o\times h p_v}$ is a square matrix.

```{.python .input  n=2}
class MultiHeadAttention(nn.Block):
    def __init__(self, hidden_size_qkv, num_heads, dropout, **kwargs):  
        super(MultiHeadAttention, self).__init__(**kwargs)
        hidden_size_out = hidden_size_qkv * num_heads
        self.num_heads = num_heads
        self.attention = d2l.DotProductAttention(dropout)
        self.W_q = nn.Dense(hidden_size_qkv, use_bias=False, flatten=False)
        self.W_k = nn.Dense(hidden_size_qkv, use_bias=False, flatten=False)
        self.W_v = nn.Dense(hidden_size_qkv, use_bias=False, flatten=False)
        self.W_o = nn.Dense(hidden_size_out, use_bias=False, flatten=False)
    ...
    def forward(self, query, key, value, valid_length):
        ...
        # (batch_size, seq_len, hidden_size_qkv * num_heads) to
        # (batch_size * num_heads, seq_len, hidden_size_qkv).
        ...
        # Transpose from (batch_size * num_heads, seq_len, hidden_size_qkv) back to
        # (batch_size, seq_len, hidden_size_qkv * num_heads)
...
def transpose_qkv(X, num_heads):
    # Original X shape: (batch_size, seq_len, hidden_size_qkv * num_heads),
    # -1 means inferring its value, 
    # after first reshape, X shape: (batch_size, seq_len, num_heads, hidden_size_qkv)
    X = X.reshape(X.shape[0], X.shape[1], num_heads, -1)
    
    # After transpose, X shape: (batch_size, num_heads, seq_len, hidden_size_qkv)
    X = X.transpose(0, 2, 1, 3)
    
    # Merge the first two dimensions. 
    # Use reverse=True to infer shape from right to left.
    # Output shape: (batch_size * num_heads, seq_len, hidden_size_qkv)
...
Let us validate the `MultiHeadAttention` model in the a toy example. Create a multi-head attention with the hidden size $d_o = 100$, the output will share the same batch size and sequence length as the input, but the last dimension will be equal to `hidden_size` $\times$ `num_heads` $= 100 \times 10 = 1000$.
...
Below, the `PositionWiseFFN` shows how to implement a position-wise FFN with two dense layers of hidden size `hidden_size_1` and `hidden_size_2`, respectively.

```{.python .input  n=5}
class PositionWiseFFN(nn.Block):
    def __init__(self, hidden_size_1, hidden_size_2, **kwargs):
        super(PositionWiseFFN, self).__init__(**kwargs)
        self.ffn_1 = nn.Dense(hidden_size_1, flatten=False, activation='relu')
        self.ffn_2 = nn.Dense(hidden_size_2, flatten=False)
...
[
 {
  "data": {
   "text/plain": "array([[ 0.00094134,  0.00033891, -0.00141665, -0.00196909, -0.00091536,\n        -0.00324369, -0.0029864 , -0.0057988 ],\n       [ 0.00094134,  0.00033891, -0.00141665, -0.00196909, -0.00091536,\n        -0.00324369, -0.0029864 , -0.0057988 ],\n       [ 0.00094134,  0.00033891, -0.00141665, -0.00196909, -0.00091536,\n        -0.00324369, -0.0029864 , -0.0057988 ]])"
  },
  "execution_count": 6,
  "metadata": {},
  "output_type": "execute_result"
...
class EncoderBlock(nn.Block):
    def __init__(self, embedding_size, hidden_size, num_heads, dropout, **kwargs):
        super(EncoderBlock, self).__init__(**kwargs)
        self.attention = MultiHeadAttention(embedding_size, num_heads, dropout)
        self.addnorm_1 = AddNorm(dropout)
        self.ffn = PositionWiseFFN(embedding_size, hidden_size)
...
Due to the residual connections, this block will not change the input shape. It means the `embedding_size` argument should be equal to the input size of the last dimension.
...
class TransformerEncoder(d2l.Encoder):
    def __init__(self, vocab_size, embedding_size, hidden_size,
        ...
        for i in range(num_layers):
            self.blks.add(
                EncoderBlock(embedding_size, hidden_size, num_heads, dropout))
...
Let us create an encoder with two stacked encoder transformer blocks, whose hyper-parameters are same as before.
...
class DecoderBlock(nn.Block):
    ...
    def __init__(self, embedding_size, hidden_size, num_heads, dropout, i, **kwargs):
...
class TransformerDecoder(d2l.Decoder):
    def __init__(self, vocab_size, embedding_size, hidden_size,
        ...
        for i in range(num_layers):
            self.blks.add(
                DecoderBlock(embedding_size, hidden_size, num_heads, dropout, i))
