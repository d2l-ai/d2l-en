...
Now we will define the hidden state initialization function `init_gru_state`. Just like the `init_rnn_state` function defined in :numref:`chapter_rnn_scratch`, this function returns a tuple composed of an ndarray with a shape (batch size, number of hidden units) whose values are all zeros.
