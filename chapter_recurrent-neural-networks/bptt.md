...
Hence, we have a chain of values $\{\ldots, (h_{t-1}, x_{t-1}, o_{t-1}), (h_{t}, x_{t}, o_t), \ldots\}$ that depend on each other via recursive computation. The forward propagation is fairly straightforward. All we need is to loop through the $(x_t, h_t, o_t)$ triples one step at a time. The discrepancy between outputs $o_t$ and the desired targets $y_t$ is then evaluated by an objective function as
...
$$
\partial_{w_h}h_{t}=\partial_{w_h}f(x_{t},h_{t-1},w_h)+\partial_{h_{t-1}}f(x_{t},h_{t-1},w_h)\partial_{w_h}h_{t-1}.
$$
...
* **Truncate the sum after** $\tau$ **steps.** This is what we have been discussing so far. This leads to an *approximation* of the true gradient, simply by terminating the sum above at $\partial_{w_h} h_{t-\tau}$. The approximation error is thus given by $\partial_{h_{t-\tau-1}} f(x_{t-\tau}, h_{t-\tau-1}, w_h) \partial_{w_h} h_{t-\tau-1}$ (multiplied by a product of  gradients involving $\partial_h f$). In practice this works quite well. It is what is commonly referred to as truncated BPTT (backpropgation through time) :cite:`Jaeger.2002`. One of the consequences of this is that the model focuses primarily on short-term influence rather than long-term consequences. This is actually *desirable*, since it biases the estimate towards simpler and more stable models.

* **Randomized Truncation.** Last we can replace $\partial_{w_h} h_t$ by a random variable which is correct in expectation but which truncates the sequence. This is achieved by using a sequence of $\xi_t$ where $E[\xi_t] = 1$ and $P(\xi_t = 0) = 1-\pi_t$ and furthermore $P(\xi_t = \pi_t^{-1}) = \pi_t$. We use this to replace the gradient:

$$z_t  = \partial_{w_h} f(x_t, h_{t-1}, w_h) + \xi_t \partial_{h_{t-1}} f(x_t, h_{t-1}, w_h) \partial_{w_h} h_{t-1}.$$
