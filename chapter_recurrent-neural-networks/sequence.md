...
* It is much harder to predict tomorrow's stock prices than to fill in the blanks for one stock price we missed previouly, even though the above two scenarios are just a matter of estimating one number. After all, foresight is so much harder than hindsight. In statistics, the former scenario (predicting beyond the known observations) is called *extrapolation* whereas the latter case (estimating between the existing observations) is called *interpolation*.
